...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <div class="card" style="padding: 20px;">
            <form action="{{route('admin.blogs.modify',$blog->id)}}" method="post" enctype="multipart/form-data" id="submit_form">
                @csrf
                @method('PUT')
                <div class="mb-3">
                    <span>Blog Title</span>
                    <input class="form-control" type="text" name="title" id="" value="{{$blog->title}}">
                    <p class="input_error">{{$errors->first('title')}}</p>
                </div>
                <div class="mb-3">
                    <span>Short Description</span>
                    <textarea  class="form-control" name="short_description" id="" cols="30" rows="3">{{$blog->short_description}}</textarea>
                    <p class="input_error">{{$errors->first('short_description')}}</p>
                </div>
                <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    <div class="course_des_bg">
                        ...
                        <div class="">
                            <div class="field">
                                <div name="description" class="form-des" id="editor" contenteditable="true">@php echo $blog->body @endphp</div>
                            </div>
                        </div>
                        <input type="hidden" name="description" id="input_des">
                    </div>
                </div>
                <p class="input_error">{{$errors->first('description')}}</p>
